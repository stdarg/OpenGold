...
void persistence(){
    ...
    auto wrong_mask=profile;replace(wrong_mask,"PC17 4 4 ","PC17 4 0 ");encounter.participants[0].character_profile=wrong_mask;rejects([&]{(void)rules->create(encounter,13);});
    replace(profile,"PC17","PC16");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{99,"vanguard","Target",1,{5,1}}}},13);});
