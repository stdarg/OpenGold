...
var originals := {}
var locales := ["en", "es"]
const SLOT = "Rogue advancement test"
func _initialize() -> void:
    for arg in OS.get_cmdline_user_args():
        if arg == "--rogue-demo": locales = ["en"]
        if arg.begins_with("--rogue-fixture="): fixture = arg.trim_prefix("--rogue-fixture=")
        if arg.begins_with("--rogue-captures="): captures = arg.trim_prefix("--rogue-captures=")
        if arg.begins_with("--rogue-output="): output = arg.trim_prefix("--rogue-output=")
...
func run_checks() -> void:
    ...
    for locale in locales:
        ...
        for attained in [2,3,4]:
            ...
            require(level.visible and str(attained) in level.get_node("Title").text, "Ordinary level-up opens for next Rogue level")
            if attained >= 3:
                require("Thief" in level.get_node("Note").text if locale == "en" else "Ladrón" in level.get_node("Note").text, "Unimplemented subclass scope stays explicit")
            if attained == 4:
                require(not level.get_node("Feat").disabled and level.get_node("Ability1").selected == 2, "Existing level-four controls default to Dexterity ASI")
            ...
            await capture("rogue-level-" + str(attained) + "-" + locale, level)
        ...
        await press("SaveSlots/Action"); await press("SaveSlots/Action")
        require(not current_scene.get_node("SaveSlots").visible, "Advanced Rogue saves through ordinary party flow")
    ...
    restore_files(); print("Rogue advancement UI checks passed"); quit(0)
