...
void persistence(){auto rules=custom();auto c=battle(*rules,hero());act(*c,"chill_touch",2);auto saved=c->save();auto forged=saved;forged.replace(forged.find(module()->identity().version),6,"0.6.42");rejects([&]{(void)rules->restore(forged);});check(c->save()==saved,"Invalid restore preserves session");
...
void stable_continuation(){
    ...
    auto copy=rules->restore(c->save());auto checkpoint=c->save();checkpoint.replace(checkpoint.find(module()->identity().version),6,"0.6.43");rejects([&]{(void)rules->restore(checkpoint);});
