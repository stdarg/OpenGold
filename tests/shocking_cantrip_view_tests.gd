...
func _initialize() -> void:
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--shocking-capture="):
			captures = arg.trim_prefix("--shocking-capture=")
...
func run_checks() -> void:
	...
	for locale in ["en", "es"]:
		...
		await press("Back")
		await press("Next")
		require(blast.text.contains("Shocking Grasp" if locale == "en" else "Toque electrizante"), "Localized cantrip")
		for size in [Vector2i(1120, 800), Vector2i(1920, 1080)]:
			root.size = size
			await settle()
			await capture("shocking-choices-" + locale + "-" + str(size.x))
	TranslationServer.set_locale("en")
	await press("Next")
	var name: LineEdit = current_scene.get_node("Name")
	name.text = "Shocking Wizard"
	name.text_changed.emit(name.text)
	await press("Next")
	await press("Next")
	await press("Modifiers")
	require(current_scene.get_node("ModifiersModal/Text").text.contains("Shocking Grasp"), "Final sheet retains selected spell")
	print("Shocking creator checks passed")
	quit(0)
