...
void access(){auto creation=srd5::character_rules();auto rules=module();auto d=draft();
    ...
    auto profile=rules->character_profile(h.sheet(),{}).data;check(profile.starts_with("PC13 1 0 130 "),"New recipe records sourced Cleric access");profile.replace(0,4,"PC11");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{2,"vanguard","Target",1,{3,1}}}},13);});
