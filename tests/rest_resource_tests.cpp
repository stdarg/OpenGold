...
void persistence_and_advancement(){
    ...
    check(checkpoint.starts_with("OGCOMBAT 23 ")&&rules->restore(checkpoint)->save()==checkpoint,"Combat checkpoint stores remaining dice exactly");
...
void old_saves(){
    ...
    check(rewritten.substr(rewritten.find('\n',rewritten.find('\n')+1)+1)==test::with_alert_grants(test::with_sneak_attack_grants(test::with_action_surge_grants(test::with_initial_wizard_spell_grants(expected),{true})))+"1 0 ","Campaign migration adds sourced spell/Alert grants, the empty rest window and module identity, preserving all original training, resources, effects, equipment and timers");
