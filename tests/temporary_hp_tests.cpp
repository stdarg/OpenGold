...
void actual_combat(){
    ...
    auto corrupt=saved;const auto at=corrupt.find(" 0 \"\"\n",corrupt.find("PC8"));check(at!=corrupt.npos,"Current actor has the empty pool suffix");corrupt.replace(at,6," -1 \"x\"\n");
...
std::string upgraded(std::string bytes){
    ...
    check(rows[0].starts_with("OGCOMBAT 10 "),"Fixture was generated by the old writer");rows[0].replace(9,2,"11");rows[0].replace(rows[0].find("0.6.13"),6,module()->identity().version);
...
void old_writer(){
    ...
    check(next.substr(next.find('\n',next.find('\n')+1)+1)==body,"Campaign migration preserves every old field including fixed Dwarf grants and clocks");
}
}
