...
void actual_combat(){
    ...
    auto corrupt=saved;const auto at=corrupt.find(" 0 \"\" 0 0",corrupt.find("PC27"));check(at!=corrupt.npos,"Current actor has the empty pool suffix");corrupt.replace(at,9," -1 \"x\" 0 0");
...
void campaign(){
    ...
    party.complete_training(active,*srd5::character_rules(),{{"origin:languages",{"elvish","orc"}},{"class:fighter:fighting_style",{"archery"}},{"class:fighter",{"athletics","history"}},{"background:soldier:gaming_set",{"dice"}}});check(pool(party.member(active).character,party.member(active).vitals).amount==8,"Training completion preserves Temporary HP");
