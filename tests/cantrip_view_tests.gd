...
func run_checks() -> void:
	...
	require(current_scene.get_node("SpellChoices/Rows/Pending").visible and not current_scene.get_node("Next").disabled, "Unavailable third choice stays pending without blocking creation")
	await capture("cantrip-choices-1120")
	await press("Next")
	require(current_scene.get_node("PageTitle").text == "Name", "Name follows Spell Choices")
	await press("Back")
	require(fire.button_pressed and poison.button_pressed, "Back retains both selections")
	await press("Back")
	await press("Next")
	require(fire.button_pressed and poison.button_pressed, "Training navigation retains cantrips")
	await press("Next")
	...
	require(sheet.contains("Poison Spray") and sheet.contains("Fire Bolt") and sheet.contains("1 cantrips"), "Created character sheet shows choices and remaining entitlement")
