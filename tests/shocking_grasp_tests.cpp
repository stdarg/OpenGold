...
void persistence_guards(){auto rules=custom();auto c=battle(*rules,hero());act(*c,"shocking_grasp",2);const auto current=c->save();auto old=current;old.replace(old.find("0.6.44"),6,"0.6.37");rejects([&]{(void)rules->restore(old);});
