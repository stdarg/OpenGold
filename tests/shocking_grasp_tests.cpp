...
void access(){auto rules=module();auto h=hero();const auto access=rules->spell_access(h.sheet());check(access.cantrips.size()==1&&access.cantrips[0].id=="shocking_grasp"&&access.cantrips[0].source_id=="class:wizard:spellcasting","Real Wizard source");
    auto profile=rules->character_profile(h.sheet(),{}).data;check(profile.starts_with("PC32 1 0 1028 "),"Selected bit in versioned recipe");profile.replace(0,4,"PC25");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{2,"vanguard","Enemy",1,{2,1}}}},13);});
...
void reaction_and_armor(){auto rules=custom();bool covered=false;
    ...
    auto c=rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Wizard",0,{1,1},rules->character_profile(hero().sheet(),{}).data},{2,"campaign-character","Plate Fighter",1,{2,1},rules->character_profile(fighter.sheet(),std::vector<std::string>{"plate"}).data}}},89);while(c->snapshot().actor!=1)act(*c,"end");const auto random=rng(*c);const int hp=unit(*c,2).hit_points;act(*c,"shocking_grasp",2);check(rng(*c)==random+2*0x9e3779b97f4a7c15ULL&&unit(*c,2).hit_points==hp-8,"Metal armor grants no Advantage; no ability modifier to damage");
}
...
void persistence_guards(){auto rules=custom();auto c=battle(*rules,hero());act(*c,"shocking_grasp",2);const auto current=c->save();auto old=current;old.replace(old.find(module()->identity().version),6,"0.6.37");rejects([&]{(void)rules->restore(old);});
