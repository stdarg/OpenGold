...
void campaign_and_migration(){
    ...
    const auto grip=body.find("2 2 \"feature:fighting_style\"");
    check(grip!=body.npos,"Single-member fixture stores grip before two Fighter grants");body[grip]='3';
    std::uint64_t checksum=14695981039346656037ULL;for(unsigned char c:body){checksum^=c;checksum*=1099511628211ULL;}
    rejects([&]{(void)decode_campaign("OPENGOLD-CAMPAIGN 8\n"+std::to_string(checksum)+"\n"+body,*srd5::character_rules(),*rules,"grip",nullptr);});
