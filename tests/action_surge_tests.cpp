...
void legacy(){auto rules=module();auto creation=srd5::character_rules();auto old=read("campaign-v11-surge.ogs");CampaignParty p(module());p.restore(decode_campaign(old,*creation,*rules,"surge",nullptr).party);
    ...
    check(body(upgraded)==test::with_action_surge_grants(expected,{true,false,true}),"Every old campaign byte is retained except module identity and the two justified grants");CampaignParty again(module());again.restore(decode_campaign(upgraded,*creation,*rules,"surge",nullptr).party);check(encode_campaign(again,nullptr,"surge")==upgraded,"Migration is canonical");
