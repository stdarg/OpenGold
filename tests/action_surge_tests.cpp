...
void campaign(){auto rules=module();auto creation=srd5::character_rules();
    for(bool npc:{false,true}){CampaignParty p(module());auto h=hero();h.inventory().add("longsword","Longsword");const auto id=npc?p.recruit("surge:companion",h):p.add_pc(h);p.equip(id,1);p.award_experience(2700,"surge-xp");p.advance(id,p.default_advancement(id));auto c=battle(p.member(id).character,{"longsword"});act(*c,"action_surge");act(*c,"dash");p.begin_combat();p.apply_combat(c->snapshot());p.end_combat();check(remaining(p.member(id).character,p.member(id).vitals)==0,"Campaign handoff preserves spent resource");
        const auto bytes=encode_campaign(p,nullptr,"surge");CampaignParty copy(module());copy.restore(decode_campaign(bytes,*creation,*rules,"surge",nullptr).party);check(encode_campaign(copy,nullptr,"surge")==bytes,"Campaign replay retains grants, state and history");copy.complete_training(id,*creation,{{"origin:languages",{"elvish","orc"}},{"class:fighter:fighting_style",{"archery"}},{"class:fighter",{"athletics","history"}}});check(remaining(copy.member(id).character,copy.member(id).vitals)==0,"Training completion does not refund uses");copy.advance(id,copy.default_advancement(id));check(remaining(copy.member(id).character,copy.member(id).vitals)==0,"Campaign advancement preserves expenditure");
