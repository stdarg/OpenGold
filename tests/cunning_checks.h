...
void run(){
    ...
    auto forged=hit->save();replace(forged,"0.6.41","0.6.34");rejects([&]{(void)rules->restore(forged);});forged=hit->save();replace(forged,"OGCOMBAT 15","OGCOMBAT 14");rejects([&]{(void)rules->restore(forged);});
    ...
    rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.41","0.6.34"),*creation,*rules,"cunning",nullptr);});
