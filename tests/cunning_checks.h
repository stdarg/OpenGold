...
void run(){
    ...
    auto forged=hit->save();replace(forged,module()->identity().version,"0.6.34");rejects([&]{(void)rules->restore(forged);});forged=hit->save();replace(forged,"OGCOMBAT 15","OGCOMBAT 14");rejects([&]{(void)rules->restore(forged);});
