...
void run(){
    ...
    auto forged=hit->save();replace(forged,module()->identity().version,"0.6.34");rejects([&]{(void)rules->restore(forged);});forged=hit->save();replace(forged,forged.starts_with("OGCOMBAT 23 ")?"OGCOMBAT 23":forged.starts_with("OGCOMBAT 22 ")?"OGCOMBAT 22":"OGCOMBAT 21","OGCOMBAT 14");rejects([&]{(void)rules->restore(forged);});
    ...
    rejects([&]{(void)decode_campaign(corrupt(bytes,module()->identity().version,"0.6.34"),*creation,*rules,"cunning",nullptr);});
