...
void run(){
    ...
    for(const auto* race:{"human","orc","dwarf","goliath"}){
        ...
        auto profile=rules->character_profile(h.sheet(),{}).data;replace(profile,"PC25","PC23");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{99,"vanguard","Enemy",1,{5,5}}}},2);});
    ...
    auto forged=hit->save();replace(forged,"0.6.37","0.6.34");rejects([&]{(void)rules->restore(forged);});forged=hit->save();replace(forged,"OGCOMBAT 15","OGCOMBAT 14");rejects([&]{(void)rules->restore(forged);});
    ...
    rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.34"),*creation,*rules,"cunning",nullptr);});
