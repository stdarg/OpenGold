...
void access(){auto creation=srd5::character_rules();auto rules=module();auto d=draft();
    ...
    auto profile=rules->character_profile(hero().sheet(),{}).data;check(profile.starts_with("PC12 1 0 69 "),"Explicit cantrip mask belongs to new recipe");profile.replace(0,4,"PC10");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{2,"vanguard","Enemy",1,{3,1}}}},13);});
...
void legacy(){auto rules=module();auto creation=srd5::character_rules();const auto base=root/"tests/fixtures";const auto old=read(base/"campaign-v10-poison.ogs");CampaignParty p(module());p.restore(decode_campaign(old,*creation,*rules,"poison",nullptr).party);
    ...
    check(saved.substr(saved.find('\n',saved.find('\n')+1)+1)==test::with_legacy_cantrip_choices(expected),"Actual old writer gains only an absent choices field and module identity");
