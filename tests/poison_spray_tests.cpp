...
void access(){auto creation=srd5::character_rules();auto rules=module();auto d=draft();
    ...
    auto profile=rules->character_profile(hero().sheet(),{}).data;check(profile.starts_with("PC28 1 0 69 "),"Explicit cantrip mask belongs to new recipe");profile.replace(0,4,"PC10");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},profile},{2,"vanguard","Enemy",1,{3,1}}}},13);});
