...
std::string upgraded(std::string bytes){const auto at=bytes.find("0.6.19");check(at!=bytes.npos,"Actual prior writer identity");bytes.replace(at,6,"0.6.20");bytes.replace(9,2,"13");return bytes+"0\n";}
void legacy(){auto rules=module(false);const auto base=root/"tests/fixtures";const auto old=read(base/"campaign-v10-savage.ogs");CampaignParty p(module(false));p.restore(decode_campaign(old,*srd5::character_rules(),*rules,"savage-fixture",nullptr).party);auto expected=old.substr(old.find('\n',old.find('\n')+1)+1);expected.replace(expected.find("0.6.19"),6,"0.6.20");const auto bytes=encode_campaign(p,nullptr,"savage-fixture");check(bytes.substr(bytes.find('\n',bytes.find('\n')+1)+1)==expected,"All existing campaign grants, equipment, wounds, pools and clocks unchanged");
