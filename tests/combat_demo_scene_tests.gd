...
func check_demo() -> void:
    ...
    require(is_equal_approx(combat.get_node("EffectAudio").volume_linear, 0.25),
        "Movement and death effects use quarter volume")
    root.push_input(active_key)
    require(combat.selected_character_cell() == Vector2i(8, 5),
    ...
    require(is_equal_approx(combat.get_node("AttackAudio").volume_linear, 0.25),
        "Attack effects use quarter volume")
