...
void invalid(){auto rules=module();const auto base=hero().sheet();const auto test=[&](CharacterSheet s){rejects([&]{(void)rules->character_profile(s,{});});};
    ...
    auto profile=rules->character_profile(base,{}).data;check(profile.starts_with("PC25 1 0 5 "),"New combat recipe carries sourced access");auto bad=profile;bad.replace(0,4,"PC9");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Wizard",0,{1,1},bad},{99,"vanguard","Target",1,{5,1}}}},13);});
