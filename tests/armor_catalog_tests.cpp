...
void legacy(){
    ...
    check(bytes.substr(bytes.find('\n',bytes.find('\n')+1)+1)==test::with_initial_wizard_spell_grants(expected),"Prior campaign gains only explicit spell/Sage grants and module identity, preserving grants, original provenance, wounds, pools and time");
    ...
    const auto previous=read(fixtures/"combat-v12-armor.save");auto c=rules->restore(previous);auto same=previous;same.replace(same.find("0.6.17"),6,rules->identity().version);check(c->save()==test::with_savage_choice(same),"Prior combat migration preserves exact state");
