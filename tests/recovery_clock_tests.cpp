...
std::string upgraded(std::string_view bytes){
    auto lines=rows(bytes);check(lines[0].starts_with("OGCOMBAT 9 "),"Frozen writer is combat nine");lines[0].replace(9,1,"11");
    ...
    for(unsigned i=4;i<8;++i){lines[i]+=lines[i].starts_with("2 ")?" 4500 0":" 0 0";lines[i]+=" 0 \"\"";}
    return join(lines);
}
void frozen_saves(){
