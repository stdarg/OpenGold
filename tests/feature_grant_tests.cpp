...
void creation(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class)){
        ...
        check(rules->character_profile(sheet,{}).data.starts_with("PC17 1 2 "),"Creation grant supplies Savage Attacker to combat without a level-four feat");
...
void profiles_and_migration(){
    ...
    wrong=profile;replace(wrong,"PC17 4 2 ","PC17 4 0 ");rejects([&]{(void)rules->create(encounter(wrong),13);});
