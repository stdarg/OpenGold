...
func run_checks() -> void:
	...
	change_scene_to_file("res://scenes/startup.tscn")
	await settle()
	require(current_scene.name == "Startup", "--lang must pause startup")
	...
	require(config.load("user://settings.cfg") == OK and config.get_value("interface", "language") == "es", "Spanish preference was not saved")
	if args.has("--splash"):
	...
	require(config.save("user://settings.cfg") == OK, "Cannot prepare restart check")
