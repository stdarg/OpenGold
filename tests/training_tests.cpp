...
void all_class_skills(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto bg:{"acolyte","criminal","sage","soldier"}){
        ...
        for(const auto& first:expected){
            ...
            if(klass.id!="rogue")rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.29"),*creation,*rules,"class-skills-new",nullptr);});
        }
    }
...
void bard_instruments(){
    ...
    replace(profile,"PC25","PC19");rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Bard",0,{1,1},profile},{99,"vanguard","Enemy",1,{6,6}}}},13);});
    CampaignParty party(module());party.add_pc(h);const auto bytes=encode_campaign(party,nullptr,"bard-new");
    rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.30"),*creation,*rules,"bard-new",nullptr);});
...
void monk_tools(){
    ...
    for(const auto& id:expected){
        ...
        replace(encounter.participants[0].character_profile,"PC25","PC20");rejects([&]{(void)rules->create(encounter,13);});
        rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.31"),*creation,*rules,"monk-new",nullptr);});
...
void druid_herbalism(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto* background:{"sage","acolyte","criminal","soldier"}){
        ...
        replace(encounter.participants[0].character_profile,"PC25","PC21");rejects([&]{(void)rules->create(encounter,13);});
        CampaignParty party(module());party.add_pc(h);const auto bytes=encode_campaign(party,nullptr,"druid-new");
        rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.32"),*creation,*rules,"druid-new",nullptr);});
    }
...
void soldier_gaming(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto& variant:variants){
        ...
        rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.33"),*creation,*rules,"gaming-new",nullptr);});
        const auto profile=rules->character_profile(sheet,{}).data;
        ...
        replace(encounter.participants[0].character_profile,"PC25","PC22");rejects([&]{(void)rules->create(encounter,13);});
...
void sage_training(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class)){
        ...
        if(klass.id=="rogue"){
            ...
            auto pc15=original;replace(pc15,"PC25","PC15");
            ...
            auto checkpoint=prior->save();replace(checkpoint,"0.6.37","0.6.26");
            ...
            auto old=original;replace(old,"PC25","PC14");
    ...
    auto forged_combat=combat->save();replace(forged_combat,"0.6.37","0.6.25");rejects([&]{(void)rules->restore(forged_combat);});
    ...
    for(const auto& bad:{corrupt(current,"skill:arcana","skill:nature"),corrupt(current,"0.6.37","0.6.25")})
...
void remaining_backgrounds(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto background:{"acolyte","soldier"}){
        ...
        auto old=rules->character_profile(sheet,{}).data;replace(old,"PC25","PC15");
        rejects([&]{(void)rules->create({{8,8,std::vector<std::uint8_t>(64)},{{1,"campaign-character","Forged",0,{1,1},old},{2,"vanguard","Enemy",1,{5,1}}}},1);});
    }
    ...
    rejects([&]{(void)decode_campaign(corrupt(current,"0.6.37","0.6.26"),*creation,*rules,"backgrounds-migration",nullptr);});
    auto members=party.participants();members[0].cell={1,1};members[1].cell={2,1};members.push_back({99,"vanguard","Enemy",1,{6,6}});
    const auto combat=rules->create({{8,8,std::vector<std::uint8_t>(64)},members},42);
    check(rules->restore(combat->save())->save()==combat->save(),"PC25 preserves combat continuation");
    auto forged=combat->save();replace(forged,"0.6.37","0.6.26");rejects([&]{(void)rules->restore(forged);});
...
void starting_styles(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto style:{"defense","archery"}){
        ...
        replace(profile,"PC25","PC17");encounter.participants[0].character_profile=profile;rejects([&]{(void)rules->create(encounter,13);});
    ...
    rejects([&]{(void)decode_campaign(corrupt(bytes,"0.6.37","0.6.28"),*creation,*rules,"style-migration",nullptr);});
}
