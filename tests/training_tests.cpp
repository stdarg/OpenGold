...
void all_class_skills(){
    ...
    auto body=[](const auto& s){return s.substr(s.find('\n',s.find('\n')+1)+1);};auto expected=body(prior);replace(expected,"0.6.29",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void druid_herbalism(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto* background:{"sage","acolyte","criminal","soldier"}){
        ...
        replace(encounter.participants[0].character_profile,profile.substr(0,profile.find(' ')),"PC21");rejects([&]{(void)rules->create(encounter,13);});
    ...
    auto body=[](const auto& text){return text.substr(text.find('\n',text.find('\n')+1)+1);};auto expected=body(prior);replace(expected,"0.6.32",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void soldier_gaming(){
    ...
    auto body=[](const auto& text){return text.substr(text.find('\n',text.find('\n')+1)+1);};auto expected=body(prior);replace(expected,"0.6.33",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void preset_training(){
    ...
    for(unsigned i=0;i<pool.size();++i){
        ...
        if(c.creation_data().background=="criminal")check(std::count(c.sheet().grants.begin(),c.sheet().grants.end(),FeatureGrant{"feat:alert","background:criminal",1,{}})==1,"Criminal presets receive exactly one fixed Alert grant");
...
void remaining_backgrounds(){
    ...
    auto expected=body(bytes);replace(expected,"0.6.26",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void starting_styles(){
    ...
    for(const auto& klass:creation->choices(CreationField::character_class))for(const auto style:{"defense","archery"}){
        ...
        invalid=d;invalid.training["class:fighter:fighting_style"]={"dueling"};rejects([&]{(void)hero(invalid);});
    ...
    auto body=[](const auto& s){return s.substr(s.find('\n',s.find('\n')+1)+1);};auto expected=body(old);replace(expected,"0.6.28",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void cunning_prior_writer(){
    ...
    auto body=[](const auto& text){return text.substr(text.find('\n',text.find('\n')+1)+1);};auto expected=body(prior);replace(expected,"0.6.34",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void monk_tool_prior_writer(){
    ...
    auto body=[](const auto& text){return text.substr(text.find('\n',text.find('\n')+1)+1);};
    auto expected=body(saved);replace(expected,"0.6.31",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
...
void bard_instrument_prior_writer(){
    ...
    auto body=[](const auto& text){return text.substr(text.find('\n',text.find('\n')+1)+1);};
    auto expected=body(saved);replace(expected,"0.6.30",rules->identity().version);expected=test::with_alert_grants(test::with_sneak_attack_grants(test::with_tactical_mind_grants(expected)));
