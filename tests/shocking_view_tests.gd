...
func _initialize() -> void:
    for arg in OS.get_cmdline_user_args():
        if arg.begins_with("--shocking-fixtures="): fixtures = arg.trim_prefix("--shocking-fixtures=")
        if arg.begins_with("--shocking-capture="): captures = arg.trim_prefix("--shocking-capture=")
...
func run_checks() -> void:
    ...
    for locale in ["en", "es"]:
        ...
        require(spells.item_count == 1 and spells.get_item_metadata(0) == "shocking_grasp", "Only the learned spell is listed")
        require(spells.get_item_text(0) == ("Shocking Grasp" if locale == "en" else "Toque electrizante"), "Localized spell choice")
        ...
        for size in [Vector2i(1120, 800), Vector2i(1920, 1080)]:
            ...
            if not captures.is_empty():
                ...
                require(root.get_texture().get_image().save_png(captures.path_join("shocking-" + locale + "-" + str(size.x) + ".png")) == OK, "Rendered controls captured")
