...
void legacy(){
    ...
    check(bytes.substr(bytes.find('\n',bytes.find('\n')+1)+1)==test::with_initial_wizard_spell_grants(body),"Campaign migration adds explicit spell/Sage grants and preserves all Dwarf/Orc grants, state, equipment and clock");
