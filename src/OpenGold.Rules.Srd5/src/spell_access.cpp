...
FeatureGrant grant(std::string_view id,unsigned level){return {"spell:"+std::string(id),std::string(source),level,{{"access",find(id).level?"spellbook":"cantrip"}}};}
}
...
unsigned wizard_casting_mask(const SpellAccess& access){
    unsigned result=0;for(const auto& s:access.cantrips)result|=find(s.id).mask;
