...
Definition character_definition(std::string_view bytes)
{
    ...
    const bool with_instruments=magic=="PC20";
...
std::unique_ptr<Session> Session::restore(std::shared_ptr<const Content> content, std::string_view bytes)
{
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if(module_before(identity,{0,6,27})&&(actor.source.character_profile.starts_with("PC16 ")||actor.source.character_profile.starts_with("PC17 ")||(actor.source.character_profile.starts_with("PC18 ")||(actor.source.character_profile.starts_with("PC19 ")||actor.source.character_profile.starts_with("PC20 ")))))
            throw std::runtime_error("Legacy checkpoint cannot contain the completed fixed-background profile");
        if(module_before(identity,{0,6,28})&&(actor.source.character_profile.starts_with("PC17 ")||(actor.source.character_profile.starts_with("PC18 ")||(actor.source.character_profile.starts_with("PC19 ")||actor.source.character_profile.starts_with("PC20 ")))))
            throw std::runtime_error("Legacy checkpoint cannot contain an Archery profile");
        if(module_before(identity,{0,6,29})&&(actor.source.character_profile.starts_with("PC18 ")||(actor.source.character_profile.starts_with("PC19 ")||actor.source.character_profile.starts_with("PC20 "))))
            throw std::runtime_error("Legacy checkpoint cannot contain a starting-style profile");
        if(module_before(identity,{0,6,31})&&actor.source.character_profile.starts_with("PC20 "))
            throw std::runtime_error("Legacy combat cannot contain Bard instrument profiles");
        if(module_before(identity,{0,6,30})&&(actor.source.character_profile.starts_with("PC19 ")||actor.source.character_profile.starts_with("PC20 ")))
...
public:
    ...
    bool accepts_campaign_identity(const Identity& saved) const override {
        if(saved.version!=content_->identity.version&&saved.version!="0.3.0"&&saved.version!="0.4.0"&&saved.version!="0.5.0"&&saved.version!="0.6.0"&&saved.version!="0.6.1"&&saved.version!="0.6.2"&&saved.version!="0.6.3"&&saved.version!="0.6.4"&&saved.version!="0.6.5"&&saved.version!="0.6.6"&&saved.version!="0.6.7"&&saved.version!="0.6.8"&&saved.version!="0.6.9"&&saved.version!="0.6.10"&&saved.version!="0.6.11"&&saved.version!="0.6.12"&&saved.version!="0.6.13"&&saved.version!="0.6.14"&&saved.version!="0.6.15"&&saved.version!="0.6.16"&&saved.version!="0.6.17"&&saved.version!="0.6.18"&&saved.version!="0.6.19"&&saved.version!="0.6.20"&&saved.version!="0.6.21"&&saved.version!="0.6.22"&&saved.version!="0.6.23"&&saved.version!="0.6.24"&&saved.version!="0.6.25"&&saved.version!="0.6.26"&&saved.version!="0.6.27"&&saved.version!="0.6.28"&&saved.version!="0.6.29"&&saved.version!="0.6.30")return false;
    ...
    CharacterProfile character_profile(const CharacterSheet& sheet,std::span<const std::string> gear,EquipmentState equipment={}) const override {
        ...
        std::ostringstream out;out<<"PC20 "<<sheet.level<<' '<<features<<' '<<spells<<' '<<std::quoted(sheet.character_class)<<' '<<std::quoted(sheet.race);
...
std::unique_ptr<RulesModule> parse_content(std::string_view content_bytes)
{
    ...
    Content content;content.identity={"opengold.srd5","0.6.31",revision+"/"+std::to_string(hash)};
