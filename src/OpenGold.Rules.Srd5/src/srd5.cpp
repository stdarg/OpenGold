...
struct Definition {
    ...
    bool dwarf{},cunning{},tactical_mind{},champion{},great_weapon_fighting{},two_weapon_fighting{};
...
Definition character_definition(std::string_view bytes,std::optional<std::span<const std::string>> equipment_override=std::nullopt)
{
    ...
    const bool with_alert=magic=="PC40";
    const bool with_mastery=magic=="PC39"||with_alert;
    const bool with_light=magic=="PC38"||with_mastery;
    const bool with_hands=magic=="PC37"||with_light;
    ...
    if(!in||(magic!="PC1"&&magic!="PC2"&&!selected)||level<1||level>(selected?4u:2u)||features>(with_alert?63u:with_light?31u:with_style_routes?15u:with_archery?7u:3u)||selected_spells>(with_chill?4095u:with_shocking?2047u:with_warlock?1023u:with_frost?511u:with_cleric_cantrips?255u:with_cantrips?127u:63u)||std::any_of(scores.begin(),scores.end(),[](int n){return n<3||n>20;}))
        throw std::runtime_error("Invalid character profile");
    if(level>1&&klass!="Fighter"&&klass!="Cleric"&&klass!="Wizard"&&!(with_cunning&&klass=="Rogue"&&(level==2||with_rogue))&&!(with_style_routes&&(klass=="Paladin"||klass=="Ranger")))throw std::runtime_error("Advancement is unsupported for this class");
    ...
    d.alert=(features&32)!=0;d.ac=10+dex;d.initiative=dex+(d.alert?int(2+(level-1)/4):0);d.speed=race=="Goliath"?35:30;d.level=level;
    ...
    if(magic=="PC35"&&klass!="Rogue")throw std::runtime_error("Rogue profile requires Rogue class");
    d.sneak_level=with_rogue&&klass=="Rogue"?level:0;d.great_weapon_fighting=(features&8)!=0;d.two_weapon_fighting=(features&16)!=0;
    ...
    if(selected){
        ...
        if(selected_spells&~allowed||(features&1)&&klass!="Fighter"&&!(with_style_routes&&(klass=="Paladin"||klass=="Ranger")))throw std::runtime_error("Invalid prepared spells or feat prerequisites");
    ...
    if(magic=="PC6"||with_training){
        ...
        const auto effects=detail::validate_grants(features_only,detail::grant_source_id(klass),detail::grant_source_id(race),background,level,magic=="PC8"||magic=="PC9"||with_spells,magic=="PC9"||with_spells,with_surge,with_archery,with_styles,with_mind,with_champion,with_arcane,with_rogue,with_style_routes,with_light,with_alert);
        ...
        : content_(std::move(content)), board_(std::move(encounter.battlefield)), rng_(seed), scope_(encounter.scope)
    {
        ...
        if(!restoring){
            for(const auto& a:actors_){
                for(const auto& key:a.definition.equipment_keys)if(const auto* w=detail::weapon(key);w&&(w->thrown||w->light))physical_inventory_=true;
                for(const auto& item:a.source.inventory)if(const auto* w=detail::weapon(item.definition);w&&(w->thrown||w->light))physical_inventory_=true;
            }
            for(const auto& a:actors_)if(a.definition.other_weapon)physical_inventory_=true;
        ...
        if(!restoring&&std::any_of(actors_.begin(),actors_.end(),[](const auto& a){return a.definition.two_weapon_fighting;}))activate_light();
        if(!restoring){
            if(outcome_==Outcome::ongoing)for(const auto& a:actors_)
                if(a.source.side==0&&def(a).alert&&conscious(a))initiative_choices_.push_back(a.source.id);
            if(initiative_choices_.empty())start_encounter_turns();
        }
    }
...
private:
    ...
    void start_encounter_turns(){
        ...
        if(outcome_==Outcome::ongoing&&!begin_turn())end_turn();
    }
    void resolve_initiative(const Command& command){
        if(command.verb=="initiative_swap")std::swap(actor(command.actor).initiative,actor(command.target).initiative);
...
{
    const auto eligible=[&](const auto& item){return item.holder==a.source.id&&!item.stowed&&detail::weapon(item.definition);};
    if(a.selected_weapon)for(const auto& item:items_)if(item.id==a.selected_weapon&&eligible(item))return item.id;
    for(const auto& item:items_)if(eligible(item))return item.id;
    return 0;
}
void Session::activate_light()
{
    if(light_active_)return;
    physical_inventory_=true;initialize_items();
    const auto count=items_.size();
    for(unsigned i=0;i<count;++i)if(items_[i].holder&&!items_[i].stowed&&items_[i].quantity>1){
        auto carried=items_[i];carried.id=unsigned(items_.size()+1);--carried.quantity;carried.stowed=true;
        items_[i].quantity=1;items_.push_back(std::move(carried));
    }
    light_active_=true;
}
Actor Session::item_actor(const Actor& a,unsigned token) const
{
    ...
    if(item.holder!=a.source.id||item.stowed||!detail::weapon(item.definition))throw std::runtime_error("Weapon is not held");
...
bool Session::light_eligible(const Actor& a,unsigned token) const
{
    ...
    return weapon&&weapon->light&&std::any_of(a.light_origins.begin(),a.light_origins.end(),[&](auto prior){return prior!=token;});
}
...
{
    const auto reaches=[&](int reach){return distance(a.source.cell,from)<=reach&&distance(a.source.cell,to)>reach;};
    if(reaches(def(a).reach))return true;
    for(const auto& item:items_)if(item.holder==a.source.id&&!item.stowed)
        if(const auto* w=detail::weapon(item.definition);w&&reaches(w->ranged?5:w->reach))return true;
    return false;
}
bool Session::weapon_reaction(const Actor& a,const Definition& weapon) const
{
    if(!pending()||path_index_>=path_.size())return false;
    return distance(a.source.cell,actors_[turn_].source.cell)<=weapon.reach&&distance(a.source.cell,path_[path_index_])>weapon.reach;
}
void Session::validate_light() const
{
    ...
    for(const auto& a:actors_){
        ...
        if(a.light_origins.size()>1&&(!a.surge_used||a.actions.normal||a.actions.surge))throw std::runtime_error("Too many qualifying Attack actions");
        std::set<unsigned> seen;
        for(auto id:a.light_origins){
            if(!id||id>items_.size()||!seen.insert(id).second)throw std::runtime_error("Invalid qualifying weapon identity");
            const auto* w=detail::weapon(items_[id-1].definition);if(!w||!w->light)throw std::runtime_error("Qualifying weapon is not Light");
        }
    }
}
...
Definition Session::equipped_definition(const Actor& a,const std::vector<HeldItemView>& items) const
{
    ...
    if(a.selected_weapon)for(const auto& item:items)if(item.id==a.selected_weapon&&item.holder==a.source.id&&!item.stowed)keys.push_back(item.definition);
    for(const auto& item:items)if(item.holder==a.source.id&&!item.stowed&&item.id!=a.selected_weapon)keys.push_back(item.definition);
...
void Session::drop_held(Actor& a)
{
    ...
    if(changed){a.selected_weapon=0;a.definition=equipped_definition(a,items_);a.weapon_hands=a.definition.weapon_hands;}
}
...
Message Session::throw_label(const Actor& a,const HeldItemView& item) const
{
    ...
    const bool full=std::count_if(items_.begin(),items_.end(),[&](const auto& i){return i.holder==a.source.id&&!i.stowed;})>=2;
    if(item.stowed&&full)for(const auto& held:items_)if(held.id==held_weapon(a)){
...
unsigned Session::throw_weapon(Actor& a,Actor& target,unsigned token,bool light)
{
    ...
    const bool full=std::count_if(items_.begin(),items_.end(),[&](const auto& i){return i.holder==a.source.id&&!i.stowed;})>=2;
    if(!light&&selected.stowed&&full){const auto stow=held_weapon(a);for(auto& held:items_)if(held.id==stow)held.stowed=true;if(a.selected_weapon==stow)a.selected_weapon=0;}
    auto attacker=thrown_actor(a,selected.definition);attacker.light_damage=light;
...
Snapshot Session::snapshot() const
{
    ...
    if(!initiative_choices_.empty())s.actor=initiative_choices_.front();
    ...
    for(const auto& a:actors_) {
        ...
        for(const auto& item:items_)if(item.holder==a.source.id)if(const auto* w=detail::weapon(item.definition)){
            ...
            if(w->light&&!a.light_origins.empty()){
                ...
                if(!item.stowed&&w->ranged)option("light_ranged","Light ranged attack — {weapon} ({hand})");
                if(w->thrown)option("light_throw","Light throw — {weapon} ({hand})");
            }
        }
...
std::vector<Command> Session::legal_commands() const
{
    ...
    if(!initiative_choices_.empty()){
        for(const auto id:initiative_choices_){const auto& a=actor(id);
            add(id,"initiative_keep","Keep initiative");
            for(const auto& ally:actors_)if(ally.source.id!=id&&ally.source.side==a.source.side&&conscious(ally))
    ...
    const auto add_weapons=[&](const Actor& a,bool reacting){
        for(const auto& item:items_)if(item.holder==a.source.id&&!item.stowed&&detail::weapon(item.definition)&&item.id!=held_weapon(a)){
...
{
    const auto& d=def(a);
    return d.great_weapon_fighting&&!ranged&&!d.ranged_weapon&&!d.weapon_label.empty()&&a.weapon_hands==2&&(d.versatile_sides||d.weapon_hands==2)
        ?detail::DamageDieRule::great_weapon_fighting:detail::DamageDieRule::normal;
}
    ...
    while(path_index_<path_.size()&&a.hp>0) {
        ...
        if(reactors_.empty()&&!a.disengaged)for(const auto& other:actors_)
            if(other.source.side!=a.source.side&&conscious(other)&&other.reaction&&!detail::opportunity_blocked(other.effects)&&
                has_weapon_reaction(other,a.source.cell,destination)&&
...
bool Session::submit(const Command& command)
{
    ...
    if(!initiative_choices_.empty()){resolve_initiative(command);
    }else if(!graze_&&!weapon_hit_&&!champion_move_&&effect_waiting()){use_effect(command);
    }else if(graze_){
        ...
        if(command.verb=="dash"){a.movement+=d.speed;++a.dashes;log(a.source.name+" dashes.",{"{name} dashes.",{{"name",a.source.name}}});}
            ...
            const auto* w=detail::weapon(items_.at(command.item-1).definition);if(w&&w->light)activate_light();
            const auto used=throw_weapon(a,actor(command.target),command.item);if(light_active_)qualify_light(a,used);
        }
        else {
            // Only a weapon Attack action earns Light; spells and unarmed fallback do not.
            const auto selected_key=std::find_if(d.equipment_keys.begin(),d.equipment_keys.end(),[](const auto& key){return detail::weapon(key)!=nullptr;});
            const auto* weapon=selected_key==d.equipment_keys.end()?nullptr:detail::weapon(*selected_key);
            const bool qualifies=weapon&&weapon->light&&(command.verb=="ranged"||(command.verb=="melee"&&!weapon->ranged));
            if(qualifies)activate_light();
            const auto token=held_weapon(a);
            if(physical_inventory_&&command.verb=="ranged"&&weapon&&weapon->thrown&&token){
                const auto used=throw_weapon(a,actor(command.target),token);if(qualifies)qualify_light(a,used);
            }else{attack(a,actor(command.target),command.verb!="melee",command.verb=="fire_bolt");if(qualifies)qualify_light(a,token);}
    ...
    update_outcome();
    if(initiative_choices_.empty()&&outcome_==Outcome::ongoing&&!pending()&&!weapon_hit_&&!champion_move_&&!effect_waiting()&&actors_[turn_].hp==0)end_turn();
...
std::string Session::save() const
{
    ...
    if(format>=26){out<<initiative_choices_.size();for(const auto id:initiative_choices_)out<<' '<<id;out<<'\n';}
    return out.str();
}
...
{
    if(initiative_choices_.empty())return;
    if(round_!=1||turn_!=0||elapsed_ms_||outcome_!=Outcome::ongoing||pending()||!path_.empty()||temporary_offer_||weapon_hit_||check_choice_||champion_move_||graze_||effect_waiting())
        throw std::runtime_error("Invalid pre-turn Initiative phase");
    std::set<EntityId> seen;
    for(const auto id:initiative_choices_){const auto& a=actor(id);
        if(!seen.insert(id).second||a.source.side!=0||!def(a).alert||!conscious(a))throw std::runtime_error("Invalid Initiative holder");
    }
    for(std::size_t i=0;i<actors_.size();++i){const auto& a=actors_[i];
        if(!a.actions.normal||a.actions.surge||!a.bonus||!a.reaction||a.dodge||a.surge_used||a.rush_used||a.savage_used||a.sneak_used||a.aim_used||a.moved||a.cleave_used||a.movement!=def(a).speed)
            throw std::runtime_error("Spent action before Initiative decisions");
        if(i&&(actors_[i-1].initiative<a.initiative||(actors_[i-1].initiative==a.initiative&&actors_[i-1].source.id>a.source.id)))throw std::runtime_error("Unsorted Initiative phase");
    }
}

void Session::validate_restored_state(bool legacy_facing_reaction) const
{
    validate_initiative();validate_weapon_hit();validate_check();validate_champion_move();validate_graze();validate_mastery_state();
    ...
    if (outcome_ != expected || (initiative_choices_.empty() && expected == Outcome::ongoing && mover.hp == 0&&!champion_move_&&!effect_waiting()&&!(weapon_hit_&&weapon_hit_->cleave&&effect_reaction_origin_)))
...
std::unique_ptr<Session> Session::restore(std::shared_ptr<const Content> content, std::string_view bytes)
{
    ...
    if(version>=26&&module_before(identity,{0,6,61}))throw std::runtime_error("Legacy combat cannot contain Initiative choices");
    ...
    for (unsigned i = 0; i < count; ++i) {
        ...
        if(module_before(identity,{0,6,56})&&actor.source.character_profile.starts_with("PC39 "))throw std::runtime_error("Legacy checkpoint cannot contain Weapon Mastery");
        if(module_before(identity,{0,6,55})&&(version>=22||actor.source.character_profile.starts_with("PC38 ")))throw std::runtime_error("Legacy checkpoint cannot contain Light attack state");
        ...
        if(module_before(identity,{0,6,61})&&actor.source.character_profile.starts_with("PC40 "))throw std::runtime_error("Legacy checkpoint cannot contain Alert profiles");
    ...
    if(version>=26){unsigned count{};input>>count;
        if(!input||!count||count>session->actors_.size())throw std::runtime_error("Invalid Initiative choice count");
        for(unsigned i=0;i<count;++i){EntityId id{};input>>id;session->initiative_choices_.push_back(id);}
    }
...
public:
    ...
    bool accepts_campaign_identity(const Identity& saved) const override {
        if(saved.version!=content_->identity.version&&saved.version!="0.6.60"&&saved.version!="0.6.59"&&saved.version!="0.6.58"&&saved.version!="0.6.57"&&saved.version!="0.6.56"&&saved.version!="0.6.55"&&saved.version!="0.6.54"&&saved.version!="0.6.53"&&saved.version!="0.3.0"&&saved.version!="0.4.0"&&saved.version!="0.5.0"&&saved.version!="0.6.0"&&saved.version!="0.6.1"&&saved.version!="0.6.2"&&saved.version!="0.6.3"&&saved.version!="0.6.4"&&saved.version!="0.6.5"&&saved.version!="0.6.6"&&saved.version!="0.6.7"&&saved.version!="0.6.8"&&saved.version!="0.6.9"&&saved.version!="0.6.10"&&saved.version!="0.6.11"&&saved.version!="0.6.12"&&saved.version!="0.6.13"&&saved.version!="0.6.14"&&saved.version!="0.6.15"&&saved.version!="0.6.16"&&saved.version!="0.6.17"&&saved.version!="0.6.18"&&saved.version!="0.6.19"&&saved.version!="0.6.20"&&saved.version!="0.6.21"&&saved.version!="0.6.22"&&saved.version!="0.6.23"&&saved.version!="0.6.24"&&saved.version!="0.6.25"&&saved.version!="0.6.26"&&saved.version!="0.6.27"&&saved.version!="0.6.28"&&saved.version!="0.6.29"&&saved.version!="0.6.30"&&saved.version!="0.6.31"&&saved.version!="0.6.32"&&saved.version!="0.6.33"&&saved.version!="0.6.34"&&saved.version!="0.6.35"&&saved.version!="0.6.36"&&saved.version!="0.6.37"&&saved.version!="0.6.38"&&saved.version!="0.6.39"&&saved.version!="0.6.40"&&saved.version!="0.6.41"&&saved.version!="0.6.42"&&saved.version!="0.6.43"&&saved.version!="0.6.45"&&saved.version!="0.6.44"&&saved.version!="0.6.46"&&saved.version!="0.6.47"&&saved.version!="0.6.48"&&saved.version!="0.6.49"&&saved.version!="0.6.50"&&saved.version!="0.6.51"&&saved.version!="0.6.52")return false;
    ...
    std::vector<std::string> supported_features() const override{return {"alert","light","two_weapon_fighting","loading","scholar","arcane_recovery","champion","stabilize","tactical_mind","chill_touch","shocking_grasp","eldritch_blast","initiative","movement","unconscious_enemy_transit","melee","ranged","critical_hits","dodge","dash","disengage","opportunity_attacks","facing","turn_opportunity_attacks","death_saves","second_wind","action_surge","cunning_dash","cunning_disengage","sneak_attack","steady_aim","great_weapon_fighting","ray_of_frost","fire_bolt","poison_spray","sacred_flame","cure_wounds","magic_missile","healing_word","scorching_ray","level_two_slots","manual_advancement","ability_score_improvement","defense","archery","savage_attacker","saving_throws","blinded","blindness","timed_effects","versatile","feature_grants","training_grants","rest_resources","hit_dice","recovery_clocks","campaign_recovery","typed_damage","damage_affinities","dwarven_poison_resistance","temporary_hp","adrenaline_rush","heavy_weapons","weapon_catalog","armor_catalog","wizard_spellbook","somatic_components","checkpoint"};}
    ...
    AdvancementOptions advancement_options(const CharacterSheet& sheet) const override
    {
        ...
        if(sheet.character_class=="Fighter"||((sheet.character_class=="Paladin"||sheet.character_class=="Ranger")&&result.level==2)){
            if(sheet.character_class=="Fighter")result.fighting_styles.push_back({"keep","Keep current","Retain the class-granted Fighting Style."});
            for(auto style:detail::fighting_styles()){
                style.available=(sheet.character_class!="Fighter"||std::any_of(sheet.grants.begin(),sheet.grants.end(),[](const auto& g){return g.source_id=="class:fighter:fighting_style";}))&&std::none_of(sheet.grants.begin(),sheet.grants.end(),[&](const auto& g){return g.id=="feat:"+style.id;});
        ...
        if(sheet.character_class=="Fighter"&&result.level==4)result.training={detail::mastery_options("fighter",4,sheet.grants)};
        ...
        if(sheet.character_class=="Paladin"||sheet.character_class=="Ranger")result.description="Fighting Style and fixed HP advancement. Level four grants an available feat or ability points.\nSpellcasting, cantrip alternatives, mastery and other class/subclass features remain unavailable.";
        if(result.level==4)result.feats={
            ...
            {"defense","Defense","+1 AC while wearing armor. Requires the Fighting Style feature.",detail::has_grant(sheet.grants,"feature:fighting_style")&&!detail::has_grant(sheet.grants,"feat:defense")},
            ...
            {"great_weapon_fighting","Great Weapon Fighting","Treat damage dice showing 1 or 2 as 3 with an eligible Melee weapon held in two hands.",detail::has_grant(sheet.grants,"feature:fighting_style")&&!detail::has_grant(sheet.grants,"feat:great_weapon_fighting")},
            {"two_weapon_fighting","Two-Weapon Fighting","Add your ability modifier to the extra attack granted by the Light property.",detail::has_grant(sheet.grants,"feature:fighting_style")&&!detail::has_grant(sheet.grants,"feat:two_weapon_fighting")},
            {"alert","Alert","Add proficiency to Initiative; optionally swap Initiative with an eligible ally before the first turn.",!detail::has_grant(sheet.grants,"feat:alert")}};
    ...
    AdvancementOptions advancement_options(const CharacterSheet& sheet,const AdvancementChoice& choice) const override {
        ...
        if(choice.fighting_style&&sheet.character_class=="Fighter"){
            ...
            for(auto& feat:options.feats)if(feat.id=="defense"||feat.id=="archery"||feat.id=="great_weapon_fighting"||feat.id=="two_weapon_fighting")
                feat.available=feat.id!=*choice.fighting_style&&std::none_of(sheet.grants.begin(),sheet.grants.end(),[&](const auto& g){return g.id=="feat:"+feat.id&&g.source_id!=source;});
        }
        return options;
    }
    AdvancementChoice default_advancement(const CharacterSheet& sheet) const override
    {
        ...
        if(options.level==2&&(sheet.character_class=="Paladin"||sheet.character_class=="Ranger"))choice.fighting_style="defense";
        ...
        if(options.level==4){
            choice.feat="ability_score_improvement";const unsigned primary=(sheet.character_class=="Fighter"||sheet.character_class=="Paladin")?0:sheet.character_class=="Cleric"?4:(sheet.character_class=="Rogue"||sheet.character_class=="Ranger")?1:3;
    ...
    {
        const auto options=advancement_options(sheet,choice);if(!options.level)return false;
        const bool half_style=(sheet.character_class=="Paladin"||sheet.character_class=="Ranger")&&options.level==2;
        if(half_style&&!choice.fighting_style)throw std::runtime_error("Choose a Fighting Style");
        if(choice.fighting_style&&std::none_of(options.fighting_styles.begin(),options.fighting_styles.end(),[&](const auto& f){return f.id==*choice.fighting_style&&f.id!="keep"&&f.available;}))throw std::runtime_error("Choose an eligible Fighting Style");
        ...
        if(half_style)next.grants.push_back({"feature:fighting_style","class:"+detail::grant_source_id(sheet.character_class),2,{}});
        if(choice.fighting_style){
            const auto source="class:"+detail::grant_source_id(sheet.character_class)+":fighting_style";
            std::erase_if(next.grants,[&](const auto& g){return g.source_id==source;});
            next.grants.push_back({"feat:"+*choice.fighting_style,source,unsigned(next.level),{}});
        }
        for(const auto& [id,values]:choice.training){
            ...
            for(const auto& value:values)next.grants.push_back({(id=="class:wizard:scholar"?"expertise:":"mastery:")+value,id,unsigned(next.level),{}});
        }
        ...
        if(next.character_class=="Paladin"||next.character_class=="Ranger"){
            const std::string note="Fighting Style and fixed HP advancement. Level four grants an available feat or ability points.\nSpellcasting, cantrip alternatives, mastery and other class/subclass features remain unavailable.";
            next.class_modifiers+="\n"+note;next.class_messages.push_back({note,{}});
        }else{
    ...
    void validate_saved_grants(const Identity& saved,const CharacterSheet& sheet,std::span<const FeatureGrant> grants) const override {
        ...
        if(module_before(saved,{0,6,53})&&(sheet.character_class=="Paladin"||sheet.character_class=="Ranger")&&sheet.level>1)throw std::runtime_error("Legacy campaign cannot contain advanced Paladins/Rangers");
        ...
        if(module_before(saved,{0,6,56})&&std::any_of(grants.begin(),grants.end(),detail::is_mastery_grant))throw std::runtime_error("Legacy campaign cannot contain Weapon Mastery");
        if(module_before(saved,{0,6,55})&&detail::has_grant(grants,"feat:two_weapon_fighting"))throw std::runtime_error("Legacy campaign cannot contain Two-Weapon Fighting");
        if(module_before(saved,{0,6,53})&&std::any_of(grants.begin(),grants.end(),[](const auto& g){return g.id=="feat:great_weapon_fighting"||(g.source_id=="class:fighter:fighting_style"&&g.level>1);}))throw std::runtime_error("Legacy campaign cannot contain new style choices");
        if(module_before(saved,{0,6,61})&&detail::has_grant(grants,"feat:alert"))throw std::runtime_error("Legacy campaign cannot grant Alert");
        auto expected=saved.version=="0.6.8"?detail::without_training(sheet.grants):sheet.grants;
        if(module_before(saved,{0,6,61}))std::erase_if(expected,[](const auto& g){return g.id=="feat:alert";});
    ...
    CharacterProfile character_profile(const CharacterSheet& sheet,std::span<const std::string> gear,EquipmentState equipment={}) const override {
        ...
        if(features&8)result.item_modifiers+="Great Weapon Fighting: eligible Melee weapon damage dice showing 1 or 2 count as 3 while held in two hands.\n";
        if(features&32)result.item_modifiers+="Alert: add proficiency to Initiative; optionally swap Initiative with an eligible ally before the first turn.\n";
        ...
        if(features&8)result.item_messages.push_back({"Great Weapon Fighting: eligible Melee weapon damage dice showing 1 or 2 count as 3 while held in two hands.",{}});
        if(features&32)result.item_messages.push_back({"Alert: add proficiency to Initiative; optionally swap Initiative with an eligible ally before the first turn.",{}});
...
std::unique_ptr<RulesModule> parse_content(std::string_view content_bytes)
{
    ...
    Content content;content.identity={"opengold.srd5","0.6.61",revision+"/"+std::to_string(hash)};
