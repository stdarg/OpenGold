...
enum class TrainingPolicy { legacy, sage, all_backgrounds, fighter_style, class_skills, bard_instruments, monk_tools, druid_herbalism, soldier_gaming, scholar, style_routes };
...
std::vector<rules::FeatureGrant> training_grants(std::string_view klass,std::string_view background,const rules::TrainingChoices& choices,TrainingPolicy policy=TrainingPolicy::style_routes);
...
rules::TrainingChoices training_choices(std::span<const rules::FeatureGrant> grants,std::string_view klass,std::string_view background,TrainingPolicy policy=TrainingPolicy::style_routes);
rules::TrainingProfile training_profile(std::span<const rules::FeatureGrant> grants,std::string_view klass,
    std::string_view background,unsigned level,const std::array<int,6>& scores,TrainingPolicy policy=TrainingPolicy::style_routes);
