...
enum class TrainingPolicy { legacy, sage, all_backgrounds, fighter_style, class_skills };
std::vector<rules::TrainingChoiceGroup> training_options(const rules::CharacterDraft& draft);
std::vector<rules::FeatureGrant> training_grants(std::string_view klass,std::string_view background,const rules::TrainingChoices& choices,TrainingPolicy policy=TrainingPolicy::class_skills);
...
rules::TrainingChoices training_choices(std::span<const rules::FeatureGrant> grants,std::string_view klass,std::string_view background,TrainingPolicy policy=TrainingPolicy::class_skills);
rules::TrainingProfile training_profile(std::span<const rules::FeatureGrant> grants,std::string_view klass,
    std::string_view background,unsigned level,const std::array<int,6>& scores,TrainingPolicy policy=TrainingPolicy::class_skills);
