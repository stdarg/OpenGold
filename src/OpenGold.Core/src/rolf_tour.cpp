...
bool RolfTourSession::camp(RestKind kind)
{
    ...
    if(!town_||!campaign_||snapshot_.phase!=TourPhase::completed||campaign_->in_combat()||campaign_->state().short_rest||campaign_->state().spell_rest||campaign_->state().training_rest||campaign_->state().rest_activity)return false;
    resuming_camp_=false;camp_kind_=kind;begin_event(2);advance(0);return true;
}
bool RolfTourSession::resume_camp()
{
    if(!town_||!campaign_||!can_leave()||campaign_->in_combat()||campaign_->state().short_rest||campaign_->state().spell_rest||campaign_->state().training_rest||
...
bool RolfTourSession::explore(ExplorationCommand command)
{
    ...
    if (snapshot_.phase != TourPhase::completed||(campaign_&&(campaign_->in_combat()||campaign_->state().short_rest||campaign_->state().spell_rest||campaign_->state().training_rest||campaign_->state().rest_activity))) return false;
    if (town_) {
