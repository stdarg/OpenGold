...
template<class Translate> void refresh_cantrip_controls(godot::Node& parent,const opengold::CharacterCreator& creator,
    ...
    rows->get_node<Label>("Count")->set_text(tr(N_("Wizard cantrips"))+" ("+String::num_uint64(picked.size())+" / "+String::num_uint64(group.count)+")");
    ...
    for(const auto& option:group.options){
        ...
        check->set_text(tr(option.label));check->set_tooltip_text(tr(option.description));
