...
void CharacterCreationView::party_check()
{
    ...
    if(pool_check_stage_<3){
        if(pool_check_stage_==0){show_pool();if(pool_.size()!=48)throw std::runtime_error("Pool must contain four characters per class");}
            ...
            if(capture_){const auto image=get_node<Window>("PoolModal")->get_texture()->get_image();if(image.is_valid())image->save_png(ProjectSettings::get_singleton()->globalize_path("user://checks/character-pool.png"));}
    ...
    case 0:{
        creator_->select(rules::CreationField::race,OS::get_singleton()->get_cmdline_user_args().has("--adrenaline-check")?"orc":"dwarf");creator_->select(rules::CreationField::character_class,"fighter");
        recommend_portrait();creator_->roll();creator_->name(OS::get_singleton()->get_cmdline_user_args().has("--adrenaline-check")?"Party check Orc fighter":"Party check Dwarf fighter");
    ...
    case 3:
        ...
        if(!get_node<RichTextLabel>("ModifiersModal/Text")->get_text().contains(OS::get_singleton()->get_cmdline_user_args().has("--adrenaline-check")?"Adrenaline Rush":"Resistance to Poison damage"))throw std::runtime_error("Created Dwarf is missing its sourced damage resistance");
...
void CharacterCreationView::defeat_check()
{
    ...
    if(defeat_check_stage_==2){
        ...
        auto* fight=get_node<CombatView>("CampaignCombat");
        const auto broken=std::filesystem::u8path(ProjectSettings::get_singleton()->globalize_path("user://checks/save-check/defeat-corrupt.ogs").utf8().get_data());
    ...
    if(defeat_check_stage_==3){
        ...
        if(capture_){const auto image=dialog->get_texture()->get_image();if(image.is_null()||image->save_png(ProjectSettings::get_singleton()->globalize_path("user://checks/party-defeat.png"))!=OK)throw std::runtime_error("Cannot capture defeat screen");}
