...
void CharacterCreationView::capture_save_ui(){
    if(++save_capture_frames_==4){
        ...
        if(image.is_null()||image->save_png(ProjectSettings::get_singleton()->globalize_path("user://checks/campaign-load-dialog.png"))!=OK)throw std::runtime_error("Cannot capture save dialog");dialog->hide();
    }
