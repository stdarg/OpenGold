...
void CombatSpriteDemo::refresh_colors()
{
    ...
    get_node<Label>("Body")->set_text(gs("Weapon ")+String::num_int64(appearance_.combat_body+1)+" / 35");
...
{
    if(!loaded_)return;
    auto& index=part==0?appearance_.combat_head:appearance_.combat_body;const int count=part==0?14:35;
